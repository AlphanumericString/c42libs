...
int	tb_remove(void)
{
	...
	(ft_bs_set_raw(bitset, 0, 0b10000001),
		ft_bs_set_raw(bitset, 1, 0b10111101), ft_bs_remove(bitset, 1));
	if (!bitset || ft_bs_get_raw(bitset, 0) != 0b10000001
		|| ft_bs_get_raw(bitset, 1) != 0)
		return (ft_bs_free(&bitset), 2);
	ft_bs_free(&bitset);
	bitset = ft_bs_new(8);
	(ft_bs_set_raw(bitset, 0, 0b10000001), ft_bs_remove(bitset, 1));
	if (!bitset || bitset->bits != NULL || bitset->_capacity != 0)
		return (ft_bs_free(&bitset), 3);
	ft_bs_free(&bitset);
	bitset = ft_bs_new(4);
	(ft_bs_set_raw(bitset, 0, 255), ft_bs_remove(bitset, 999));
	if (!bitset || bitset->bits != NULL || bitset->_capacity != 0)
		return (ft_bs_free(&bitset), 5);
	ft_bs_free(&bitset);
	bitset = ft_bs_new(8);
	(ft_bs_set_raw(bitset, 0, 0b10000001), ft_bs_remove(bitset, 0));
	if (ft_bs_get(bitset, 0) != true || ft_bs_get(bitset, 7) != true)
		return (ft_bs_free(&bitset), 8);
	return (ft_bs_free(&bitset), ft_bs_remove(NULL, 4), 0);
}
