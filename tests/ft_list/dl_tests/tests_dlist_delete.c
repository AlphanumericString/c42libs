...
int	test_dlist_delete_self(void)
{
	...
	*data2 = 21;
	...
	if (list->next != NULL || list->data != data3)
		return (1);
	return (ft_list_dl_clear(&list, free), \
	ft_list_dl_delete_self(NULL, NULL), 0);
}
/*
	ft_list_dl_add_back(&list, ft_list_dl_create(data3));
		// NULL <-(42)<=>(21)<=>(63)-> NULL
...
int	test_dlistdelete_range(void)
{
	...
	*data_array[2] = 63;
	ft_list_dl_add_back(&list, ft_list_dl_create(data_array[2]));
	list2 = list->next;
	nb_deleted = ft_list_dl_delete_range(list, list->next, NULL);
	if (nb_deleted != 1 || ft_list_dl_size(list2) != 2 || *(int *)list2->data \
	!= *data_array[1] || *(int *)list2->next->data != *data_array[2])
		return (1);
	ft_list_dl_add_front(&list2, ft_list_dl_create(data_array[0]));
	list = list2;
	nb_deleted = ft_list_dl_delete_range(list->next, list->next->next, free);
	if (nb_deleted != 1 || ft_list_dl_size(list) != 2 || *(int *)list2->data \
	!= *data_array[0] || *(int *)list2->next->data != *data_array[2])
		return (1);
	return (ft_list_dl_clear(&list2, free), 0);
}
