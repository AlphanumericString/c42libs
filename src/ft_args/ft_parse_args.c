...
/*   Updated: 2024/04/21 10:29:31 by bgoulard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ssize_t parse_long_opt(char *str_op, const t_opt *opt_list)
{
	...
	while (opt_list[i].func)
	{
		...
		ft_strncmp(opt_list[i].long_name, str_op, 
			 ft_max(offset, ft_strlen(str_op))) == 0)
			return (i);
		i++;
	}
	return (-1);
...
ssize_t parse_short_opt(char *str_op, const t_opt *opt_list)
{
	...
	while (opt_list[i].func)
	{
		...
		opt_list[i].short_name == str_op[0] == 0)
			return (i);
		i++;
	}
	return (-1);
...
void run_opt_func(const t_opt opt, void *usr_control_struct, char *arg, char *arg_nxt)
{
...
int ft_parse_args(int argc, char **argv, void *usr_control_struct)
{
	...
	while (argv[i] && argv[i][0] == '-')
	{
		...
		if (opt_index == -1)
			return (ft_putstr_fd("Unkown option\n", 2), 1); // todo reference the option and prog name ft_putstrerr idk?
